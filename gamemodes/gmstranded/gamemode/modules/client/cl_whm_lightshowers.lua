...
local Rain = {}
Rain.ID = 1
Rain.Name = "Light Showers"
Rain.m_tblThunderTime = { min = 50, max = 120 }
Rain.m_bHasLightning = false

Rain.m_intIndoorFadeTime = 3
Rain.m_intVolumeOutside = 0.5
Rain.m_intVolumeInside = 0.3
Rain.m_intPitchOutside = 100
Rain.m_intPitchInside = 40

Rain.m_intMaxRainParts = 22
...
Rain.m_tblSounds = {
	...
	RainLoop = "whm/crucial_waterrain_light_loop.wav",
}
...
function Rain:Start( intDuration, intTimeOffset )
	...
	self.m_intFadeInRainTime = 10 --math.random( self.m_tblRainTime.min, self.m_tblRainTime.max )
	self.m_intFadeOutRainTime = 25 --math.random( self.m_tblRainTime.min, self.m_tblRainTime.max )
	...
	self.m_intFadeOutTime = 20 +self.m_intFadeOutRainTime --math.random( self.m_tblFadeOutTime.min, self.m_tblFadeOutTime.max ) +self.m_intFadeOutRainTime
...
function Rain:PlayThunderFX()
	...
	if CurTime() >= self.m_intLastThunderTime then
		if self.m_bHasLightning and math.random( 1, 3 ) == 1 and (not self.m_bFadingIn and not self.m_bFadingOut) then
	...
	if self.m_intLastLightFX and util.IsSkyboxVisibleFromPoint( EyePos() ) then
		...
		if time <= (self.m_intLastLightFX +0.2) then
			...
			DrawColorModify{
				...
				["$pp_colour_addg"] = 0,
				["$pp_colour_addb"] = 0.02,
				["$pp_colour_brightness"] = Lerp((time -self.m_intLastLightFX) /0.2, 0.25, 0),
		...
		DrawColorModify{
			...
			["$pp_colour_contrast"] = Lerp( self.m_intRainScaler, 1, 0.75 ),
			["$pp_colour_colour"] = 1,
			["$pp_colour_mulr"] = 0,
			["$pp_colour_mulg"] = 0,
			["$pp_colour_mulb"] = 0
