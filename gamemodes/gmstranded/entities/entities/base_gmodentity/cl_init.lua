...
function ENT:Draw( bDontDrawModel )
	...
	if ( LocalPlayer():GetEyeTrace().Entity == self.Entity && 
	     EyePos():DistToSqr( self:GetPos() ) < 65536 ) then
...
function ENT:SetLabelVariables()
	...
	self.LabelDistance = EyePos():DistToSqr( self:GetPos() )
	if ( self.LabelDistance > 65536 ) then return false end
	...
	// Alpha
	self.LabelAlpha = (1 - self.LabelDistance / 65536)^0.4
