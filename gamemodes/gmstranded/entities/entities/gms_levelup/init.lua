...
function ENT:Initialize()
	...
	self.mov = "stay"
	self.tstart = CurTime()
	self.tend = self.tstart + 1.6
	
	if SERVER then
		local trail = util.SpriteTrail(self, 0, self.skillcolor, true, 12, 4, 1, 1/(15+1)*0.5, "trails/laser.vmt")
	end
...
function ENT:PhysicsSimulate( physobj, deltatime )
	...
	local eftime = ( (self.tend - CurTime()) / 1.6 )
	self.z = ( ( ( -eftime + 1 )  * 110 ) + 10 )
	
	self.range = ( ( ( eftime ) * 40 ) + 5 )
