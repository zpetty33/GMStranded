...
function ENT:SetResourceDropInfo( rType, rAmt )
	...
	self.POwner.pcontents[rType] = (self.POwner.pcontents[rType] or 0) + rAmt
...
function ENT:GetTotalResources()
	...
	local total = 0
	for k, v in pairs( self.POwner.pcontents ) do
		if v<0 then v=0 end
		total = total + v
	end
	return total
...
function ENT:Use( ply )
	...
	net.Start("UpdateCacheTable")
		net.WriteTable( self.POwner.pcontents )
	net.Send( ply )
