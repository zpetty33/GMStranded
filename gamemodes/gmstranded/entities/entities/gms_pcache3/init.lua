...
function ENT:SetResourceDropInfo( rType, rAmt )
	...
	self.POwner.p3contents[rType] = (self.POwner.p3contents[rType] or 0) + rAmt
...
function ENT:GetTotalResources()
	...
	local total = 0
	for k, v in pairs( self.POwner.p3contents ) do
		if v<0 then v=0 end
		total = total + v
	end
	return total
...
function ENT:Use( ply )
	...
	net.Start("UpdateCacheTable")
		net.WriteTable( self.POwner.p3contents )
	net.Send( ply )
