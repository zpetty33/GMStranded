...
function ENT:Initialize()
	...
	self:SetUseType(3)
	--self:PhysicsInitSphere(40,"metal")
...
function ENT:GetTotalResources()
	...
	local total = 0
	for k, v in pairs( self.POwner.p2contents ) do
		if v<0 then v=0 end
		total = total + v
	end
	return total
...
function ENT:Use( ply )
	...
	if self.enabled == false then 
		ply:SendMessage("(SGS) This cache is disabled!", 60, Color(255, 255, 0, 255))
		ply.lastuse = CurTime()
		return 
	end

	net.Start("UpdateCacheTable")
		net.WriteTable( self.POwner.p2contents )
	net.Send( ply )
...
function ENT:CacheEnable( ply )
	self:SetColor(Color(0, 200, 255,255))
	
	self.enabled = true
	if ply then 
		self.POwner = ply
		if ply.p2contents == nil then
			ply.p2contents = {}
		end
	end
end
