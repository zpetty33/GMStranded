...
function EFFECT:Init( data )
	...
	local emitter = ParticleEmitter( self.Origin, false )
		...
		for i=0, NumParticles do
			...
			local newstart = self.Origin + ( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), 0):GetNormal() * math.Rand(0,15))
			...
			if ColorParticleStreak then
				...
				ColorParticleStreak:SetDieTime( math.Rand(0.5, 2) )
				...
				ColorParticleStreak:SetEndSize( math.random(0,1) )
