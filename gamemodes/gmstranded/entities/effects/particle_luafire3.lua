...
function EFFECT:Init( data )
	...
	self.ParticlesPerSecond, self.ParticlesToSpawn = 25, 0
...
function EFFECT:Think()
	...
	if finished then
		...
		self.Emitter:SetPos(self:LocalToWorld(Vector( 0, 0, 12 )))
	end
...
function EFFECT:Render()
	...
	while self.ParticlesToSpawn >= 1 do
		...
		--local pos = self:GetPos() + Vector(math.cos(ang), math.sin(ang)) * math.Rand(0, 4) + Vector( 0, 0, 12 )
		local pos = self.Entity:LocalToWorld(Vector( 0, 0, 12 )) + Vector(math.cos(ang), math.sin(ang)) * math.Rand(0, 4)
