...
function EFFECT:Init( data )
	...
	if dis > SGS.drawlights * 2 then return end
	...
	local emitter = ParticleEmitter( self.Origin, false )
		...
		for i=0, NumParticles do
			...
			local newstart = self.Origin + ( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), 0):GetNormal() * 25)
			newstart = newstart + Vector(0,0,math.Rand(0,30))
			...
			if ColorParticleStreak && GlowEffect then
				...
				ColorParticleStreak:SetDieTime( math.Rand(3.5,5.5) )
				...
				ColorParticleStreak:SetStartLength( 40 )
				ColorParticleStreak:SetEndLength( 0 )
				
				ColorParticleStreak:SetCollide( false )
				ColorParticleStreak:SetBounce( 1 )
				
				timer.Simple(1, function() ColorParticleStreak:SetVelocity( Vector(0,0,math.Rand(10,40)) ) ColorParticleStreak:SetGravity( Vector( 0, 0, math.Rand( 20, 75 ) ) ) end)
