...
function EFFECT:Init( data )
	...
	local emitter = ParticleEmitter( self.Origin, false )
		...
		for i=0, NumParticles do
			...
			local newstart = self.Origin + ( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0)):GetNormal() * 30)
			...
			if ColorParticleStreak && GlowEffect then
				...
				ColorParticleStreak:SetDieTime( 2 )
				...
				ColorParticleStreak:SetColor( 0,200,0 )
				...
				timer.Simple( 1, function() ColorParticleStreak:SetVelocity( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), 0):GetNormal() * 250 ) end )
