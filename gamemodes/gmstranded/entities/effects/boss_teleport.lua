...
function EFFECT:Init( data )
	...
	local NumParticles = 512
	
	local emitter = ParticleEmitter( self.Origin, false )
	
		for i=0, NumParticles do
			
			local newstart = self.Origin + ( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0)):GetNormal() * 100)
			local ColorParticleStreak = emitter:Add( 'sprites/gmdm_pickups/light', newstart )
			
			if ColorParticleStreak then
				
				ColorParticleStreak:SetVelocity( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0)):GetNormal() * math.Rand(50,150) )
				...
				local AirResistance = math.Rand(  10, 30 )
				
				ColorParticleStreak:SetAirResistance( AirResistance )
				
				local Gravity = math.Rand( 100, 350 )
				
				ColorParticleStreak:SetGravity( Vector( math.Rand( -50, 50 ), math.Rand( -50, 50 ), -math.Rand( 100, 350 ) ) )
				
				ColorParticleStreak:SetColor( Col.r, Col.g, Col.b )
				
				ColorParticleStreak:SetStartLength( 10 )
