...
function EFFECT:Init( data )
	...
	local emitter = ParticleEmitter( self.Origin, false )
		...
		for i=0, NumParticles do
			...
			local newstart = self.Origin + ( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0)):GetNormal() * math.Rand(1,5))
			local ColorParticleStreak = emitter:Add( 'sprites/gmdm_pickups/light', newstart )
			
			if ColorParticleStreak then
				
				ColorParticleStreak:SetVelocity( VectorRand():GetNormal() * math.Rand( 10, 40 ) )
				
				ColorParticleStreak:SetLifeTime( 0 )
				ColorParticleStreak:SetDieTime( math.Rand(0.4,1) )
				...
				local Gravity = math.Rand( -200, 200 )
