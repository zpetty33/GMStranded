...
function EFFECT:Init( data )
	...
	self.ParticlesPerSecond, self.ParticlesToSpawn = 36, 0
...
function EFFECT:Think()
	...
	if finished then
		...
		self:SetPos(pos)
		self.Emitter:SetPos( self:LocalToWorld(Vector( 4, 0, -3 )) )
	end
...
function EFFECT:Render()
	...
	while self.ParticlesToSpawn >= 1 do
		...
		local pos = self.Entity:LocalToWorld(Vector( 4, 0, -3 )) + Vector(math.cos(ang), math.sin(ang)) * math.Rand(0, 1.8)

		local particle = self.Emitter:Add('sprites/flamelet'..math.random(1,5), pos)
		if particle then
			particle:SetVelocity( Vector(0,0,math.random(4,8)) )
