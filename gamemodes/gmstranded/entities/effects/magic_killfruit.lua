	...
	local emitter = ParticleEmitter( self.Origin, false )
		...
		for i=0, NumParticles do
			...
			if ColorParticleStreak && GlowEffect then
				...
				ColorParticleStreak:SetDieTime( math.Rand(4, 5) )
				...
				ColorParticleStreak:SetColor( 255,0,255 )
				...
				timer.Simple( 0.2, function() ColorParticleStreak:SetVelocity( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), 0):GetNormal() * 125 ) end)
