...
function EFFECT:Init( data )
	...
	local NumParticles = 2048
	
	local emitter = ParticleEmitter( self.Origin, false )
	
		for i=0, NumParticles do
			
			local newstart = self.Origin + ( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), 0):GetNormal() * math.Rand(1,400))
			newstart = newstart + Vector(0,0,math.Rand(200,250))
			...
			if ColorParticleStreak && GlowEffect then
				...
				ColorParticleStreak:SetDieTime( math.Rand(4, 7) )
				...
				local AirResistance = 30
				
				ColorParticleStreak:SetAirResistance( AirResistance )
				
				local Gravity = math.Rand(25,250)
				
				ColorParticleStreak:SetGravity( Vector( 0, 0, -Gravity ) )
				
				ColorParticleStreak:SetColor( 150,0,255 )
				
				ColorParticleStreak:SetStartLength( 25 )
				ColorParticleStreak:SetEndLength( 0 )
				
				ColorParticleStreak:SetCollide( true )
				ColorParticleStreak:SetBounce( 1 )
				
				--timer.Simple( 1, function() ColorParticleStreak:SetGravity( Vector( 0, 0, -500 ) ) ColorParticleStreak:SetVelocity( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), 0):GetNormal() * 75 ) end)
