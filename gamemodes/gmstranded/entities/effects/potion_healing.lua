...
function EFFECT:Init( data )
	...
	local emitter = ParticleEmitter( self.Origin, false )
		...
		for i=0, NumParticles do
			...
			local newstart = self.Origin + ( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), 0):GetNormal() * 4)
			newstart = newstart + Vector(0,0,math.Rand(70,80))
			...
			if ColorParticleStreak && GlowEffect then
				...
				ColorParticleStreak:SetDieTime( math.Rand(2, 3.2) )
				...
				ColorParticleStreak:SetColor( 0,255,0 )
				...
				timer.Simple( 0.4, function() ColorParticleStreak:SetGravity( Vector( 0, 0, -500 ) ) ColorParticleStreak:SetVelocity( Vector(math.Rand(-1.0, 1.0), math.Rand(-1.0, 1.0), 0):GetNormal() * 75 ) end)
